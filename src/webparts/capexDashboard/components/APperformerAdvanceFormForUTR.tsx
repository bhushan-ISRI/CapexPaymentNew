...
const APperformerAdvanceFormForUTR: React.FC<IProps> = ({
  ...
  const getAttachments = async (capexId: string) => {
    try {
      ...
      console.log(
        `No requestor attachments found in ${REQUESTOR_DOCS_LIBRARY}`,
        error,
      );
      setAttachments([]);
    }
  ...
  const uploadUTRAttachments = async (capexId: string) => {
    ...
    try {
      ...
      console.error(
        `UTR attachment upload error (${UTR_DOCS_LIBRARY}):`,
        error,
      );
      throw error;
    }
    ...
    try {
      ...
      if (approverRemarks && approverRemarks.trim() === "") {
        await Swal.fire({
          icon: "warning",
          title: "Validation Error",
          text: "Approver Remarks cannot contain only spaces.",
          confirmButtonText: "OK",
        });
        setIsSubmitting(false);
        return;
      ...
      if (utrFiles.length > 0) {
        try {
          ...
          await Swal.fire({
            icon: "error",
            title: "Upload Failed",
            text: `UTR attachment upload to ${UTR_DOCS_LIBRARY} failed. Please try again.`,
            confirmButtonText: "OK",
          });
          setIsSubmitting(false);
          return;
